...
class _VAERNNBase(nn.Module):
    def __init__(self, obs, actions, hiddens, gaussians):
        super().__init__()
...
class VAERNN(_VAERNNBase):
    ...
    def forward(self, actions, obs): # pylint: disable=arguments-differ ### change to obs 
        """ MULTI STEPS forward.
        ...
        """
        seq_len, bs = actions.size(0), actions.size(1) # batch of actions for each time step in the sequence (t steps) T x N  x A 
        
        recon_batch, latents, logvar = self.vae(obs) # data is same things as obs 
        ...
        return mus, sigmas, logpi, rs, ds, recon_batch, latent, logvar
