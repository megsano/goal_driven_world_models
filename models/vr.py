"""
Define VAERNN model, supposed to be used as a world model
on the latent space.
"""
import torch
...
class VAERNN(_VAERNNBase):
    ...
    def forward(self, actions, gt_obs): # pylint: disable=arguments-differ
        """ MULTI STEPS forward.
        ...
        """
        #Run the VAE to produce a latent state from observations
        ...
        recon_batch, gt_obs_mu, gt_obs_sigma = self.vae(gt_obs)
        latents = (gt_obs_mu + gt_obs_sigma.exp() * torch.randn_like(gt_obs_mu)).view(BSIZE, SEQ_LEN, LSIZE)

        latents = latents.transpose(1, 0)

        #Run the RNN to produce mus, sigmas and other relevant variables
        ...
        return gt_obs_mu, gt_obs_sigma, recon_batch, latents, mus, sigmas, logpi, rs, ds

class VAERNNCell(_VAERNNBase):
    """ VAERNN model for one step forward """
    def __init__(self, latents, actions, hiddens, gaussians):
        super().__init__(latents, actions, hiddens, gaussians)
        self.rnn = nn.LSTMCell(latents + actions, hiddens)
        self.vae = VAE(3, latents)

    def forward(self, action, gt_ob, hidden): # pylint: disable=arguments-differ
        """ ONE STEP forward.
        ...
        gt_obs_mu, gt_obs_sigma = self.vae.encoder(gt_ob)
        latent = (gt_obs_mu + gt_obs_sigma.exp() * torch.randn_like(gt_obs_mu)).view(BSIZE, LSIZE)
