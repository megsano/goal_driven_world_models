...
# Loading model
vaernn_dir = join(args.logdir, 'vaernn')
vaernn_file = join(vaernn_dir, 'best.tar')
...
if exists(vaernn_file) and not args.noreload:
    vaernn_state = torch.load(vaernn_file)
    print("Loading VAERNN at epoch {} "
          "with test error {}".format(
              vaernn_state["epoch"], vaernn_state["precision"]))
    mdrnn.load_state_dict(vaernn_state["state_dict"])
    optimizer.load_state_dict(vaernn_state["optimizer"])
